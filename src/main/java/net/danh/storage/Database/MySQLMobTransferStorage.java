...
public class MySQLMobTransferStorage {
    ...
    public void createTransferTable() {
        ...
        try {
            ...
            plugin.getLogger().log(Level.SEVERE, "Unable to create MySQL mob transfer table", ex);
        } finally {
            try {
                if (ps != null) ps.close();
    ...
    public void insertTransfer(MobTransferData transferData) {
        ...
        try {
            ...
            plugin.getLogger().log(Level.SEVERE, "Unable to insert MySQL mob transfer data", ex);
        } finally {
            try {
                if (ps != null) ps.close();
                if (conn != null) conn.close();
    ...
    public List<MobTransferData> getTransferHistory(String playerName, int limit, int offset) {
        ...
        try {
            ...
            plugin.getLogger().log(Level.SEVERE, "Unable to get MySQL mob transfer history", ex);
        } finally {
            try {
                if (rs != null) rs.close();
                if (ps != null) ps.close();
    ...
    public int getTotalTransferCount(String playerName) {
        ...
        try {
            ...
            plugin.getLogger().log(Level.SEVERE, "Unable to get MySQL mob transfer count", ex);
        } finally {
            try {
                if (rs != null) rs.close();
                if (ps != null) ps.close();
    ...
    private long getTotalAmount(String column, String playerName) {
        ...
        try {
            ...
            plugin.getLogger().log(Level.SEVERE, "Unable to get MySQL mob transfer amount for " + column, ex);
        } finally {
            try {
                if (rs != null) rs.close();
                if (ps != null) ps.close();
