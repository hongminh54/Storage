...
public class CropWithdrawCommand extends CropCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 1 || "all".equalsIgnoreCase(args[1])) {
            amount = CropStorageManager.getPlayerItem(player, itemName);
        } else {
        ...
        new CropWithdraw(player, itemName, amount).doAction();
    }
    ...
    public String getUsage() {
        return "/cropstorage withdraw <item> [amount|all]";
    }

    @Override
    public String getDescription() {
        return "Withdraw crop items from CropStorage into inventory";
    }
}
