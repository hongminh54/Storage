...
public class ViewMobStorageGUI implements IGUI {
    ...
    public ViewMobStorageGUI(Player viewer, Player target) {
        this(viewer, target, 0);
    }

    public ViewMobStorageGUI(Player viewer, Player target, int page) {
        this.viewer = viewer;
        this.target = target;
        this.targetName = target.getName();
        this.currentPage = Math.max(0, page);
        this.config = File.getViewMobStorageGUIConfig();
    ...
    public Inventory getInventory(SoundContext context) {
        ...
        int itemsPerPage = Objects.requireNonNull(config.getString("items.mob_item.slot")).split(",").length;
        ...
        if (MobStorageManager.hasInvalidItems() && viewer.hasPermission("storage.mobstorage.admin")) {
            viewer.sendMessage(ChatUtils.colorizewp("&c&l[!] MobStorage Warning:"));
            viewer.sendMessage(ChatUtils.colorizewp("&e" + MobStorageManager.getInvalidItems().size()
                    + " &7invalid item(s) detected: &c" + String.join(", ", MobStorageManager.getInvalidItems())));
            viewer.sendMessage(ChatUtils.colorizewp("&7Use &e/mobstorage reload &7to see detailed errors"));
        }
        ...
        for (String itemTag : Objects.requireNonNull(config.getConfigurationSection("items")).getKeys(false)) {
            ...
            if (itemTag.equalsIgnoreCase("mob_item")) {
                setupMobItems(inventory, slot, configuredDrops, itemsPerPage);
    ...
    private void setupMobItems(Inventory inventory, String slot, List<String> configuredDrops, int itemsPerPage) {
        if (slot.contains(",")) {
            ...
            for (int i = startIndex; i < endIndex; i++) {
                ...
                if (slotIndex < slotList.size()) {
                    ...
                    if (meta != null) {
                        int amount = MobStorageManager.getPlayerItem(targetName, itemName);
                        int maxStorage = MobStorageManager.getMaxStorage(target);

                        String displayName = MobStorageManager.getItemDisplayName(itemName);
                        meta.setDisplayName(ChatUtils.colorizewp(displayName));

                        String sellable = MobStorageManager.getSellableSymbol(itemName);
                ...
                if (slot.contains(",")) {
                    for (String slotString : slot.split(",")) {
                        InteractiveItem item = new InteractiveItem(prevPageItem.clone(),
                            ...
                            player.openInventory(new ViewMobStorageGUI(viewer, target, currentPage - 1)
            ...
            for (String slotString : slot.split(",")) {
                InteractiveItem item = new InteractiveItem(backItem.clone(), Number.getInteger(slotString.trim()))
                        .onClick((player, clickType) -> {
                            ...
                            SoundManager.setShouldPlayCloseSound(player, false);
                            int viewerCurrentPage = MobStorageGUI.getPlayerCurrentPage(viewer);
                            player.openInventory(
                                    new MobStorageGUI(viewer, viewerCurrentPage).getInventory(SoundContext.SILENT));
                        });
                inventory.setItem(item.getSlot(), item);
            }
        } else {
