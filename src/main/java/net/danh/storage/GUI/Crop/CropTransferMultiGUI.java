...
public class CropTransferMultiGUI implements IGUI {
    private static final Set<String> LOGGED_KEYS = new HashSet<>();
    private static final Map<UUID, CropTransferMultiGUI> activeGUIs = new HashMap<>();
    ...
    public CropTransferMultiGUI(Player player, String targetPlayer) {
        this.player = player;
    ...
    public static CropTransferMultiGUI getActiveGUI(Player player) {
        ...
        if (itemsSection != null) {
            for (String itemKey : itemsSection.getKeys(false)) {
                if (!itemKey.equals("material_slots") && !itemKey.equals("decorates")) {
                    ConfigurationSection itemSection = itemsSection.getConfigurationSection(itemKey);
                    if (itemSection != null) {
                        for (int slot : getSlots(itemSection)) {
    ...
    private List<String> getPlayerItems() {
        return CropStorageManager.getConfiguredDrops().stream()
                .filter(itemName -> CropStorageManager.getPlayerItem(player, itemName) > 0)
    ...
    private InteractiveItem createMaterialItem(String itemName, ConfigurationSection section, int slot) {
        ...
        int maxTransferable = receiver != null ?
                CropTransferManager.getOptimalTransferAmount(player, targetPlayer, itemName, currentAmount) :
                currentAmount;
        String displayName = CropStorageManager.getItemDisplayName(itemName);
        Material material = CropStorageManager.getItemMaterial(itemName);
        ...
        try {
            ...
            synchronized (LOGGED_KEYS) {
                String key = "croptransfer.multi.material_item." + itemName;
                if (LOGGED_KEYS.add(key)) {
                    Storage.getStorage().getLogger().log(Level.WARNING,
                            "[Storage] Failed to build CropTransferMultiGUI icon for item '"
    ...
    public void handleClick(String itemKey, String itemName, ClickType clickType) {
        switch (itemKey) {
            case "material_click":
                handleMaterialClick(itemName, clickType);
                break;
    ...
    private void requestCustomAmount(String itemName) {
        ...
        setWaitingForCustomAmount(player, itemName);
        player.sendMessage(ChatUtils.colorize(
                File.getMessage().getString("cropstorage.transfer.gui_enter_amount")));
    }
    ...
    private void requestReceiverInput() {
        ...
        player.sendMessage(ChatUtils.colorize(
                File.getMessage().getString("cropstorage.transfer.gui_enter_receiver")));
    }

    private void setWaitingForCustomAmount(Player player, String itemName) {
        // Store in ChatListener maps for later retrieval
        net.danh.storage.Listeners.ChatListener.chat_crop_multi_transfer_item.put(player.getUniqueId(), itemName);
        net.danh.storage.Listeners.ChatListener.chat_crop_multi_transfer_target.put(player.getUniqueId(), targetPlayer);
    }
    ...
    private void confirmTransfer() {
        if (targetPlayer == null || targetPlayer.isEmpty()) {
            player.sendMessage(ChatUtils.colorize(
                    File.getMessage().getString("cropstorage.transfer.failed_no_receiver")));
            return;
        }

        if (selectedAmounts.isEmpty()) {
            player.sendMessage(ChatUtils.colorize(
                    File.getMessage().getString("cropstorage.transfer.failed_no_selection")));
            return;
        }
        ...
        if (!CropTransferManager.executeMultiTransfer(player, targetPlayer, selectedAmounts)) {
            player.sendMessage(ChatUtils.colorize(
                    File.getMessage().getString("cropstorage.transfer.failed_multi_all")));
        }
    ...
    private void cancelTransfer() {
        ...
        try {
            player.openInventory(new CropStorageGUI(player, 0).getInventory());
        } catch (Exception e) {
            Storage.getStorage().getLogger().warning("Error opening crop storage GUI: " + e.getMessage());
        }
    }
    ...
    private FileConfiguration getTransferMultiConfig() {
        return File.getCropTransferMultiGUIConfig();
    }
