...
public class CropTransferGUI implements IGUI {
    private static final Map<UUID, CropTransferGUI> activeGUIs = new HashMap<>();
    ...
    public CropTransferGUI(Player player, String targetPlayer, String itemName) {
        this.player = player;
    ...
    public static CropTransferGUI getActiveGUI(Player player) {
    ...
    private void requestCustomAmount() {
        ...
        player.sendMessage(ChatUtils.colorize(File.getMessage().getString("cropstorage.transfer.gui_enter_amount")));
        setWaitingForInput(player, true);
    }

    private void requestReceiverInput() {
        player.closeInventory();
        player.sendMessage(ChatUtils.colorize(File.getMessage().getString("cropstorage.transfer.gui_enter_receiver")));
    ...
    private void confirmTransfer() {
        if (targetPlayer == null || targetPlayer.isEmpty()) {
            player.sendMessage(ChatUtils.colorize(File.getMessage().getString("cropstorage.transfer.failed_no_receiver")));
            return;
        }

        SoundManager.setShouldPlayCloseSound(player, false);
        player.closeInventory();
        CropTransferManager.executeTransfer(player, targetPlayer, itemName, transferAmount);
    ...
    private void cancelTransfer() {
        ...
        try {
            player.openInventory(new CropItemStorage(player, itemName, 0).getInventory());
        } catch (Exception e) {
            Storage.getStorage().getLogger().warning("Error opening crop item storage GUI: " + e.getMessage());
        }
    }
    ...
    private FileConfiguration getTransferConfig() {
        return File.getCropTransferGUIConfig();
    }
