...
public class RecipeEditorListGUI implements IGUI {
    ...
    public RecipeEditorListGUI(Player player, int currentPage) {
        ...
        this.config = File.getRecipeEditorListGUIConfig();
    }
    ...
    private void setupRecipeItems(Inventory inventory) {
        ...
        addRecipeItemsToInventory(inventory, recipeList, slotArray, itemsPerPage);
        addNavigationButtons(inventory, totalPages);
    ...
    private void addNavigationButtons(Inventory inventory, int totalPages) {
        if (totalPages > 1) {
            if (currentPage > 0) {
    ...
    private ItemStack createRecipeEditorItem(Recipe recipe) {
        ...
        String statusKey = recipe.isEnabled() ? "crafting.status_enabled" : "crafting.status_disabled";
    ...
    private void toggleRecipeStatus(Player player, Recipe recipe) {
        ...
        String messageKey = recipe.isEnabled() ? "crafting.editor_enabled" : "crafting.editor_disabled";
        player.sendMessage(ChatUtils.colorize(File.getMessage().getString(messageKey)
                .replace("#recipe#", recipe.getName())));
    ...
    private void duplicateRecipe(Player player, Recipe recipe) {
        String newId = CraftingManager.generateUniqueId();
        Recipe duplicate = CraftingManager.duplicateRecipe(recipe.getId(), newId);

        if (duplicate != null) {
            CraftingManager.addRecipe(duplicate);
            player.sendMessage(ChatUtils.colorize(File.getMessage().getString("crafting.editor_duplicated")
                    .replace("#recipe#", recipe.getName())
                    .replace("#new_recipe#", duplicate.getName())));

            SoundManager.setShouldPlayCloseSound(player, false);
            player.openInventory(new RecipeEditorGUI(player, duplicate).getInventory(SoundContext.SILENT));
        } else {
            player.sendMessage(ChatUtils.colorize(File.getMessage().getString("crafting.editor_duplicate_failed")));
        }
    }
    ...
    private void createNewRecipe(Player player) {
        ...
        newRecipe.setName("&eNew Recipe");
        newRecipe.setCategory("default");

        CraftingManager.addRecipe(newRecipe);
        player.sendMessage(ChatUtils.colorize(File.getMessage().getString("crafting.editor_created")
