...
public class ChatUtils {
    ...
    private static final Pattern HEX_PATTERN_4 = Pattern.compile("\\[#([A-Fa-f0-9]{6})\\]");
    private static final Pattern HEX_PATTERN_5 = Pattern.compile("&<#([A-Fa-f0-9]{6})>");
    private static final Pattern HEX_PATTERN_6 = Pattern.compile("&\\{#([A-Fa-f0-9]{6})}");
    private static final Pattern HEX_PATTERN_7 = Pattern.compile("(?<![&\\u00A7])#([A-Fa-f0-9]{6})(?!\\w)");
    private static final Pattern HEX_PATTERN_AMPERSAND = Pattern.compile("&x(&[A-Fa-f0-9]){6}");
    private static final Pattern HEX_PATTERN_SECTION = Pattern.compile("\u00A7x(\u00A7[A-Fa-f0-9]){6}");
    ...
    private static @NotNull String translateHexColors(String message) {
        ...
        result = translateSeparatedHex(result, HEX_PATTERN_AMPERSAND, '&');
        result = translateSeparatedHex(result, HEX_PATTERN_SECTION, '\u00A7');

        result = translateHexPattern(result, HEX_PATTERN_1);
        result = translateHexPattern(result, HEX_PATTERN_5);
        result = translateHexPattern(result, HEX_PATTERN_6);
        ...
        result = translateHexPattern(result, HEX_PATTERN_7);
    ...
    private static @NotNull String translateSeparatedHex(String message, Pattern pattern, char separator) {
        Matcher matcher = pattern.matcher(message);
        StringBuffer buffer = new StringBuffer();
